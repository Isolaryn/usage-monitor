...
func runTests() {
    ...
    var manualSchedule = PollSchedule()
    precondition(manualSchedule.reserve(now: now, manual: false))
    precondition(!manualSchedule.reserve(now: now.addingTimeInterval(1), manual: true))
    manualSchedule.record(good, now: now)
    precondition(!manualSchedule.reserve(now: now.addingTimeInterval(59), manual: true))
    precondition(!manualSchedule.reserve(now: now.addingTimeInterval(60), manual: false))
    precondition(manualSchedule.reserve(now: now.addingTimeInterval(60), manual: true))
    manualSchedule.record(throttled, now: now.addingTimeInterval(60))
    precondition(!manualSchedule.reserve(now: now.addingTimeInterval(120), manual: true))
    precondition(manualSchedule.reserve(now: manualSchedule.nextAttempt, manual: true))
    let legacy = Data(#"{"nextAttempt":900,"failures":1,"lastError":"Claude rate-limited usage checks · retry later"}"#.utf8)
    var legacySchedule = try! JSONDecoder().decode(PollSchedule.self, from: legacy)
    precondition(!legacySchedule.reserve(now: Date(timeIntervalSinceReferenceDate: 100), manual: true))
    print("Passed: manual refresh, click throttling, in-flight exclusion and legacy rate-limit cooldowns")
